...
class Card:
    ...
    def __init__(self, id: str, name: str, power: int, keywords: List[Keyword] = None,
                 trigger: Trigger = None, effects: List[CardEffect] = None,
    ...
    def from_dict(cls, data):
        ...
        # Conversion des mots-clés de strings vers Enums
        raw_keywords = data.get("keywords", [])
        keywords = []
        for k_str in raw_keywords:
            try:
                keywords.append(Keyword(k_str))
            except ValueError:
                print(f"⚠️ Warning: Unknown keyword '{k_str}' for card {data.get('name')}")

        # Conversion du trigger de string vers Enum
        trigger_str = data.get("trigger")
        trigger = None
        if trigger_str:
            try:
                trigger = Trigger(trigger_str)
            except ValueError:
                print(f"⚠️ Warning: Unknown trigger '{trigger_str}' for card {data.get('name')}")
    ...
    def refresh_state(self):
        self.power = self.base_power
        self.keywords = list(self.base_keywords)
        if self.is_damaged and Keyword.TOUGH in self.keywords:
            self.keywords.remove(Keyword.TOUGH)
