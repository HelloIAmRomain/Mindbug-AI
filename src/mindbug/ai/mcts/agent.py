...
from mindbug.engine.core.consts import Phase, Keyword
...
class MCTSAgent(AgentInterface):
    ...
    def get_action(self, game):
        ...
        try:
            ...
            while time.time() < end_time:
                ...
                depth = 0
                # PROFONDEUR REDUITE DE 50 A 15 pour couper le bruit chaotique
                while sim_game.state.winner is None and depth < 15:
                ...
                while node is not None:
                    if winner:
                        ...
                        win_score = 1.0 if node.player_just_moved == winner_idx else 0.0
                    else:
                        # Si match nul ou profondeur max atteinte, on juge par l'avantage
                        win_score = self._evaluate_board_for(sim_game, node.player_just_moved)
    ...
    def _evaluate_board_for(self, game, player_idx):
        """
        Évalue le plateau de jeu de façon heuristique pour le joueur 'player_idx'.
        Calcule la différence de menace et normalise le score entre 0.0 et 1.0.
        """
        player = game.state.player1 if player_idx == 0 else game.state.player2
        opponent = game.state.player2 if player_idx == 0 else game.state.player1

        # Différentiel HP (Poids critique) [-3 à +3 => -30 à +30]
        hp_diff = player.hp - opponent.hp
        
        # Puissance brute au sol [-20 à +20]
        my_power = sum(c.power for c in player.board)
        opp_power = sum(c.power for c in opponent.board)
        power_diff = my_power - opp_power
        
        # Différentiel Mindbugs [-2 à +2 => -10 à +10]
        mb_diff = player.mindbugs - opponent.mindbugs

        score = (hp_diff * 10) + power_diff + (mb_diff * 5)
        
        # Normalisation arbitraire
        # Borne max estimée = 50. Si score = 0 => 0.5 (Match nul).
        normalized = 0.5 + (max(min(score, 50), -50) / 100.0)
        return normalized
