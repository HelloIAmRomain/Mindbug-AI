...
class ResourceManager:
    ...
    def get_card_image(self, card_model) -> pygame.Surface:
        ...
        set_id = getattr(card_model, "set", "FIRST_CONTACT")
        candidates = [
            # Check within the explicitly defined Extension Subfolder
            self._resource_path(os.path.join("cards", set_id, filename)),
            self._resource_path(os.path.join("cards", set_id, filename.replace(".jpg", ".png"))),
            
            # Fallback backward compatibility checks (flat folders)
