...
from mindbug.engine.utils.logger import log_error
...
class GameScreen(BaseScreen):
    ...
    def handle_events(self, events):
        for event in events:
            ...
            # FIX INITIATIVE : Clic global pour avancer
    ...
    def _update_ai(self):
        ...
        # --- CORRECTION CRITIQUE (Bug Crapaud Bombe) ---
        # On détermine si l'IA DOIT jouer en fonction du contexte (Jeu normal vs Sélection)
        
        req = self.game.state.active_request
        ai_player = self.game.state.player2
        
        ai_must_play = False

        if req:
            # CAS 1 : Une sélection est en cours (ex: Crapaud Bombe meurt)
            # L'IA ne joue QUE si c'est elle qui doit choisir.
            # Même si c'est son tour actif, si la requête s'adresse à P1, l'IA ne touche à rien.
            if req.selector == ai_player:
                ai_must_play = True
        else:
            # CAS 2 : Jeu normal (Play, Attack, Block)
            # L'IA joue si c'est son tour (index 1)
        ...
        if self.hovered_zone_id:
            ...
            if visual_zone_id == "PLAY_AREA":
                if card in self.game.state.active_player.hand:
                    is_p1 = (self.game.state.active_player == self.game.state.player1)
    ...
    def _refresh_ui_components(self):
        ...
        # Par défaut (HOTSEAT), on cache le joueur qui n'a pas le focus
        hide_p1 = (visual_idx == 1) and not debug
        hide_p2 = (visual_idx == 0) and not debug
        
        # FIX PVE : Règles de visibilité spécifiques
        if self.app.config.game_mode == "PVE":
            hide_p1 = False        # Le joueur humain voit TOUJOURS sa main
            hide_p2 = not debug    # La main de l'IA est TOUJOURS cachée (sauf debug)
    ...
    def _check_hotseat_curtain(self):
        # FIX : Pas de rideau pendant l'initiative
        if self.game.state.phase == Phase.INITIATIVE_BATTLE:
            self.show_curtain = False
            return
